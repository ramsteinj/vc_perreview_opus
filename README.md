...
**사용자 관리** 화면의 [CSV 일괄 등록]으로 여러 계정을 한 번에 만들 수 있습니다.
모달에서 양식 CSV를 내려받아 채운 뒤 올리면 됩니다.

| 컬럼 | 필수 | 형식 |
|------|------|------|
| `employee_no` | 필수 | 중복 불가 |
| `name` | 필수 | |
| `department_code` | 선택 | 활성 부서의 코드 |
| `position` | 선택 | |
| `role` | 선택 | `EMPLOYEE` 또는 `ADMIN` (기본 `EMPLOYEE`) |
| `hired_on` | 선택 | `YYYY-MM-DD` |

- UTF-8과 CP949(한국어 Excel 기본) 모두 자동 인식합니다
- **한 행이라도 오류가 있으면 아무것도 등록되지 않고** 행번호·사유가 표시됩니다
- 비밀번호는 자동 생성되며 등록 직후 화면에서 한 번만 확인·내려받을 수 있습니다

## CSV 내보내기

관리자 화면의 필터를 그대로 적용해 내려받습니다.

| 파일 | 내용 |
|------|------|
| `scores.csv` | 사번 · 성명 · 부서 · 평가자 · 1차/2차 점수 · 개인 점수 · 부서 가감 · 최종 점수 |
| `responses.csv` | 항목별 원점수와 의견까지 포함한 전체 응답 내역 (항목 수에 따라 컬럼 확장) |
| `pending.csv` | 미응답 평가자와 대상 목록 |

**UTF-8 with BOM**으로 내보내므로 Excel에서 바로 열어도 한글이 깨지지 않습니다.
미산출자는 점수 칸이 비고, `비고` 열에 사유(예: `1차 평가 미제출`)가 표시됩니다.
...
현재 단계: **전체 7단계 완료.** 로그인부터 최종 점수 산출과 CSV 내보내기까지
기능 요구사항 전체가 동작합니다.
