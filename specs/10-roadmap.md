...
- [x] **사용자 CSV 일괄 등록** (FR-A-02) — 2026-09-16
  스펙에 "선택 구현"으로 표시되어 Phase 2에서 보류했던 항목.
  검증 실패 시 전체 롤백, UTF-8/CP949 인코딩 자동 판별, 임시 비밀번호 1회 반환.

## 남은 판단 사항

구현이 아니라 결정이 필요한 항목들.

- [ ] **부서 가감 파라미터 확정** — 기준점 70 / 계수 0.20 / 한도 ±10은 합리적
  초기값이며 실제 인사 규정 확정 시 회차 설정에서 변경한다
  ([06-scoring.md](06-scoring.md) §5)
- [ ] **본인 계정 자기 비활성화 차단 여부** — 관리자가 2명 이상일 때 스스로를
  비활성화하면 즉시 로그아웃된다. UI에서는 막았으나 API는 열려 있다.
  스펙이 다루지 않아 보류 중
- [ ] **`by_department` 집계 기준** — 현재 평가자 소속 부서 기준.
  대상 부서 기준 집계도 필요하면 토글로 추가 가능
  ([05-admin-features.md](05-admin-features.md) FR-A-06 6-1)
